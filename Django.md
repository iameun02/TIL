...
##2. (프로젝트단) urls.py 작성<br>
  ...
  (2) urlpatterns 작성<br>
       - path('test/',views.test) : test로 들어온건 view 파일의 test함수로 보내라
...
<br><br>
   ✓ TIP<br>
     ...
     '/' 는 index 첫 화면을 다시 호출해 주는 역할 = path 에서 ''
<br><br>
...
> BASE_DIR + 프로젝트명/index.html 
   ...
   안 만들어도 동작은 되지만, 해당 탬플릿이 어느 app에서 사용되는 것인지 직관적으로 이해하기 위함
 

##4. Template 만들기
    앱 level과 동일한 레벨에서 templates 디렉토리 생성후 > 프로젝트명으로 된 폴더 생성후 > html 문서 작성
