...
𝑬[𝑋 bar] = 𝒎   ```모집단의 기대값은 표본의 기대값과 같다```
...
<br>
2.  표본추출방법 <br>
2) 단순임의 추출법 (Simple Random Sampling) <br>
   - 난수표를 활용해 추출 <br>
3) 계통추출법 (systematic sampling) <br>
   - 𝒌개의 구간으로 나눈후, 구간마다 순서대로 하나씩 추출 (𝒌 =모집단원소 수 / 표본수) ==> 데이터가 사이클을 가질 경우 편향의 문제가 생김 <br>
4) 집락추출법 (cluster sampling)<br>
   - 분할된 cluster 들이 자체적으로 모집단을 잘반영할 수있게 이질적인 요소들이 골고루 섞여 있어 한 cluster를 추출함 (층화추출보다 조사가 더 용이하며, 조사비용이 적게 든다.) <br>
5) 층화추출법 (stratified sampling) <br>
...
2. 구간추정
   ...
   ```[𝐿,𝑈]이 신뢰구간, 1- 𝛼 를 신뢰수준이라고 함 ```
    ...
    - 예시
      ```
      (조건1)정규분포를 따르는 모집단에 대하여 모평균𝒎 와 (조건2)분산𝜎² 정보를 알 때
      우리는 표본분포를 알수 있다. 
      표본분포의 평균은 모평균 𝒎과 동일하며 분산은 𝜎²/𝒏(표본수) 로 구할수 있다. 
      이때 표준화를 하면, 𝒁 = (𝑿 bar -𝒎) /𝜎²/𝒏(표본수)
      이며 ~𝑵[0,1]
      신뢰구간은 𝑿 bar ± 𝒁 𝛼/₂(𝜎/√𝒏)
      ```
