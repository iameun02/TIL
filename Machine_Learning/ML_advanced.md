...
1. Numpy
   - 행렬 에서의 '차원' = 딥러닝에서 'Rank'
   - 앞으로 array는 배열보다는 matrix (행렬)의 개념으로 이해하기
   - Tensor : 다차원 행렬 (결국 array, matrix)
   - Scalar (무게) = 0d array = Rank0 Tensor <br>
...
- Learning : MSE을 최소로 하도록 함수식의 a 와 b가 바뀌어져 가는 과정 (학습의 대상 :  기울기, 절편 즉, parameters 업데이트)
...
### <b>2. Gradient Decent</b>
 ...
 - Gradient Descent : Mean squared Error의 최소값을 찾아주는 방법
    ...
    - 머신러닝에서는 함수식이 𝑦 = 𝑤𝑥 + 𝑏라고 표현되며,
    𝑤,𝑏 는 파라미터로 표현한다. (파라미터 : 통계학에서 모수)
       - 𝑤(weight : 가중치)
       - 𝑏(bias : 편향, 즉 오차) <br><br>
  

- 손실함수 (loss)
   - error를 구하는 것 또한 수식이니까 함수다. 실측값과 에측값과의 차이를 구하는 함수를 Loss Function이라고 한다.
   - MSE도 실제로 제곱형태의 이차함수다.
   - MSE가 가장 최소가 되는 지점의 기울기(경사)를 미분으로 구하는것
   - MSE를 b에 대해서 편미분
   - 𝑦 = (𝑦- 𝑦 ℎ𝑎𝑡)² <br>
     →  𝑦 = (𝑦- (𝑤𝑥 + 𝑏)² <br> → 𝑦 = (𝑦- 𝑤𝑥 - 𝑏)²

   - 이차함수를 편미분 진행시 나오는 일차함수 결과값이 접선의 기울기가 된다.
   -  접선의 기울기는 두가지 정보를 가지고 있다.
      1. 부호 (이동방향결정) 
          - 원래값에서 편미분값을 (b-기울기) 뺴줘서 오른쪽으로 움직인다.
          - 양의 기울기일떄, 부호가 + 이니, 빼면 왼쪽으로 이동
            음의 기울기일때, 부호가 - 이니, 빼면 (+가 되니까) 오른쪽으로 이동
          - b의 시작점은 모른다.
  

      2. 크기
         - 크기는 우선 모른다. 그런데 크기가 작으면 문제가 없겠지만, 크기가 너무 커버리면(보폭이 커버리면) 최소점을 지나칠수가 있다는 문제점이 있다. 
         - 하지만 크기의 값은 정할 수없고, 자동으로 계산되는 값이다.
         - 그래서 보폭을 조절하기 위해 우리는, 경사하강법에서 편미분값에 매우 작은 0~1사이의 감마값을 곱해준다.
         - 그럼 크기가 비율로 감소된다. 통상 크기의 *0.01 만큼만 반영
         - 𝑤 와 𝑏 모두 마찬가지
    - 실제 이차함수는 𝑤 와 𝑏가 모두 조정이되니까 3차원 그래프며, 볼록한 평면의 형태를 가지게 된다.(각축은 𝑴𝑺𝑬,𝑤,𝑏)

- 오차를 제곱하는 이유
   - 절대값 그래프는 V형태로 되기 떄문에 뾰족한 부분에서의 미분이 불가능하다 (미분은 연속된 곳에서만 가능)
   - 제곱할때 오차가 커질수록 오차를 극대화 시켜준다.
